...
def register_holiday_exception_handlers(app):
    ...
    @app.exception_handler(HolidayUpdateError)
    async def holiday_update_exception_handler(
        request: Request,
        exc: HolidayUpdateError,
    ) -> JSONResponse:
        return JSONResponse(
            status_code=400,
            content={
                "detail": {
                    "code": "holiday_update_error",
                    "message": "Error occurred while updating the holiday.",
                }
            },
        )
